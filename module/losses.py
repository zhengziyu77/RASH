...
class HeteroFusedBCE(nn.Module):
    ...
    def forward(self, left, right, target_type, pairs, neg_pairs=None,edge_weight_dict=None):
        loss = 0.
        out, mask_dict = self.decoder(left, right, pairs, target_type)
        for edge_type, pos_out in out.items():
            ...
            if edge_weight_dict is None:
                ...
                edge_weight = edge_weight_dict[edge_type].view(-1,1)
                loss += F.binary_cross_entropy(pos_out, labels, weight=edge_weight.detach())

        if neg_pairs is not None:
            out, mask_dict = self.decoder(left, right, neg_pairs,target_type)
